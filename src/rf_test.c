...
void classRFIsingGraph(double *x, int *dimx, int *cat, int *maxcat,
  ...
  zeroDouble(counttrnew, 4 * nsample);
		...
		double * tgini =      (double *) S_alloc(mdim, sizeof(double));
		...
		int * at =            (int *) S_alloc(mdim*nsample1, sizeof(int));
		int * a =             (int *) S_alloc(mdim*nsample1, sizeof(int));
		int * b =             (int *) S_alloc(mdim*nsample1, sizeof(int));
		int * mind =          (int *) S_alloc(mdim, sizeof(int));
		...
		for(int i=0; i< (mdim-1);i++){
		  for (int j=i+1;j<mdim;j++){
		    ...
		    for (int m=0;m<mdim;m++){
		      ...
		      if (flagy == 0){
		        for(int n=0; n<nsample;n++) xnew[t+n*(mdim-2)] = x[m+n*mdim];
		        t++;
		      }
		      flagy = 0;
		    }
...
void classRF(double *x, int *dimx, int *cl, int *ncl, int *cat, int *maxcat,
	...
	oobprox, keepf, replace, stratify, trace,Ntree;
